      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-start justify-between bg-slate-50/50 dark:bg-slate-800/40">
          {loading ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-sky-100 dark:bg-sky-950/60 text-sky-800 dark:text-sky-300">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Record not found</p>
          )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {fields.map((field) => {
                  ...
                  return (
                    <div key={field.id} className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl">
                      <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">
                        {field.label}
                      </span>
                      <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 mt-0.5 block">
                        {val === undefined || val === null || val === '' ? (
                          <span className="text-slate-400 dark:text-slate-500 italic font-normal text-xs">Empty</span>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {record.relationships.map((rel) => (
                    <div key={rel.id} className="p-3 bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50 rounded-xl flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
                        <div>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{rel.company_name || 'Associated Record'}</span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 block">{rel.relationship_name}</span>
                        </div>
                      </div>
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-white dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                        Linked Account
                      </span>
                    </div>
                  ))}
            ...
            <form onSubmit={handleAddNote} className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3.5 py-2 text-xs bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-600 focus:ring-2 focus:ring-sky-100 dark:focus:ring-sky-900/40"
                />
                <button
                  type="submit"
            ...
            <div>
              ...
              <div className="space-y-3 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
                {(record.activities || []).length === 0 ? (
                  <p className="text-xs text-slate-400 dark:text-slate-500 pl-8">No activities recorded yet on this record.</p>
                ) : (
                  record.activities.map((act) => {
                    ...
                    return (
                      <div key={act.id} className="relative pl-8 text-xs">
                        <div className="absolute left-1.5 top-1 w-3 h-3 rounded-full bg-white dark:bg-slate-900 border-2 border-sky-600 dark:border-sky-400"></div>
                        <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl">
